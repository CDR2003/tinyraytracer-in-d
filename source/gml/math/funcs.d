...
@nogc int solveQuartic(T)(T a, T b, T c, T d, T e, T[] roots) pure nothrow if (isFloatingPoint!T)
{
    ...
    immutable int numRoots = solveCubic!T(b0, b1, b2, 1, resolventCubicRoots[]);
    assert(numRoots == 3);
    ...
    {
        immutable T Rsquare = R * R;
        immutable T Rrec = 1 / R;
...
unittest
{
    ...
    {
        double[3] roots;
        immutable int numRoots = solveCubic!double(-2, -3 / 2.0, 3 / 4.0, 1 / 4.0, roots[]);
        assert(numRoots == 3);
    ...
    {
        double[4] roots;
        immutable int numRoots = solveQuartic!double(0, -2, -1, 2, 1, roots[]);
...
double variance(R)(R r) if (isForwardRange!R)
{
    ...
    immutable auto avg = average(r.save); // getting the average
